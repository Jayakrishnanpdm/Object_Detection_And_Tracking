...
class ImprovedTheftDetector:
    ...
    def is_valid_theft_scenario(self, obj: TrackedObject) -> Tuple[bool, str]:
        """More lenient theft validation with better handling of timing"""
        current_time = time.time()
        ...
        # Criterion 2: Missing for minimum consecutive frames
        if obj.consecutive_missing_frames < self.config.consecutive_missing_threshold:
            print(f"   ❌ FAIL: Missing for only {obj.consecutive_missing_frames} consecutive frames")
            return False, "Missing consecutive frames insufficient"

        # Criterion 3: Total missing frames
        if obj.missing_frames < self.config.missing_frames_threshold:
            ...
            return False, "Total missing frames insufficient"

        # Criterion 4: Recent meaningful interaction (allow 60s window)
        recent_interactions = [
            i for i in obj.interaction_history 
            if current_time - i.timestamp <= 60.0
        ]

        if not recent_interactions:
            # Fallback: allow interaction that happened shortly before disappearance
            if obj.interaction_history:
                last_interaction_time = obj.interaction_history[-1].timestamp
                time_diff_disappearance = obj.last_seen_timestamp - last_interaction_time
                if time_diff_disappearance <= 10.0:
                    print(f"⚠️ Using fallback: Interaction happened {time_diff_disappearance:.1f}s before disappearance")
                else:
                    print(f"   ❌ FAIL: No recent interactions within 60 seconds")
                    return False, "No recent interactions"
            else:
                print(f"   ❌ FAIL: No interaction history at all")
        ...
        # Criterion 6: Confidence threshold
        if recent_interactions:
            max_confidence = max(i.confidence for i in recent_interactions)
        else:
            # fallback to last known interaction confidence
            max_confidence = obj.interaction_history[-1].confidence if obj.interaction_history else 0.0
    ...
    def get_theft_suspect_with_evidence(self, obj: TrackedObject) -> Tuple[Optional[int], Dict]:
        ...
        # Step 1: Prefer recent interactions within the last 15 seconds
        recent_interactions = [
            i for i in obj.interaction_history
            if current_time - i.timestamp <= 15.0 and i.confidence >= 0.2
        ]

        # Step 2: If none found, allow fallback if the object has *any* past interactions
        if not recent_interactions and obj.interaction_history:
            print("   ⚠️ No recent interactions — using fallback to last known interaction")
            recent_interactions = [max(obj.interaction_history, key=lambda x: x.timestamp)]

        # Step 3: Still no interactions at all? Then fail.
        if not recent_interactions:
            print(f"   ❌ No interactions found for {obj.label}")
            return None, {}
...
⚠️ ACTION REQUIRED: INVESTIGATE IMMEDIATELY
    ...
    def process_frame(self, frame: np.ndarray) -> np.ndarray:
        """Main processing function with enhanced logic and duplicate-filtering"""
        ...
        # detection_names: parallel list of class names

        # Update tracker (DeepSort) - the tracker implementation used previously
        # expects detections in a compatible format; keep call as-is to avoid breaking expected behavior.
        try:
            tracks = self.tracker.update_tracks(detections, frame=frame)
        except TypeError:
            # fallback: try passing only detections
            try:
                tracks = self.tracker.update_tracks(detections)
            except Exception:
                tracks = []
        except Exception:
            # If tracker fails, continue with empty tracks to avoid crashing the frame loop
            tracks = []

        # Build candidate list from tracker -> detection matching
        candidates = []  # tuples of (track_id, detected_name, bbox(ltrb), best_iou)
        for track in tracks:
            try:
                if not track.is_confirmed():
                    continue
            except Exception:
                # Some tracker implementations may not have is_confirmed(); assume confirmed
                pass
            ...
            try:
                ...
                # fallback: try to use track.to_tlbr or attributes
                if hasattr(track, "to_tlbr"):
                    ltrb = track.to_tlbr()
                else:
                    continue

            # Determine best matching detection (by IoU)
            detected_name = None
            best_iou = 0.0
            for i, det in enumerate(detections):
                ...
                if iou_score > best_iou and iou_score > 0.05:  # lowered minimal iou to be tolerant
                    best_iou = iou_score
                    detected_name = detection_names[i]
            if detected_name:
                candidates.append((track_id, detected_name, ltrb, best_iou))

        # Deduplicate candidates: keep best candidate per class (detected_name)
        best_per_class = {}
        for track_id, name, ltrb, iou_score in candidates:
            if name not in best_per_class or iou_score > best_per_class[name][2]:
                best_per_class[name] = (track_id, ltrb, iou_score)

        # Create current_detections from best_per_class
        current_detections = {}
        for name, (track_id, ltrb, iou_score) in best_per_class.items():
        ...
        # Remove overlapping duplicates (IoU > 0.7) within same class - cautious step
        to_remove = set()
        ids = list(current_detections.keys())
        for i in range(len(ids)):
            for j in range(i + 1, len(ids)):
                id_i = ids[i]
                id_j = ids[j]
                if current_detections[id_i]["name"] == current_detections[id_j]["name"]:
                    if self.iou(current_detections[id_i]["bbox"], current_detections[id_j]["bbox"]) > 0.7:
                        # Remove lower confidence
                        if current_detections[id_i]["confidence"] >= current_detections[id_j]["confidence"]:
                            to_remove.add(id_j)
                        else:
                            to_remove.add(id_i)
        for rid in to_remove:
            current_detections.pop(rid, None)

        # Update tracking structures using the filtered current_detections
        ...
        # Cleanup every 5 seconds approx (assuming ~30fps)
